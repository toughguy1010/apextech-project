                        ...
                        <tr id="task-{{ $task->id }}">
                            ...
                            <td>
                                <select name="update-task-status" class="update-task-status"
                                    data-url="{{ url('ceo/update-task-status', $task->id) }}">
                                    <option value="{{ Task::NOT_START }}"
                                        @if ($task->status == Task::NOT_START) selected @endif> {{ Task::getStatus(Task::NOT_START) }}</option>
                                    <option value="{{ Task::INPROGRESS }}"
                                        @if ($task->status == Task::INPROGRESS) selected @endif> {{ Task::getStatus(Task::INPROGRESS) }}</option>
                                    <option value="{{ Task::INPROGRESS }}"
                                        @if ($task->status == Task::TESTING) selected @endif> {{ Task::getStatus(Task::TESTING) }}</option>
                                    <option value="{{ Task::INPROGRESS }}"
                                        @if ($task->status == Task::COMPLETE) selected @endif>{{ Task::getStatus(Task::COMPLETE) }}</option>
                                    <option value="{{ Task::INPROGRESS }}"
                                        @if ($task->status == Task::NOTCOMPLETE) selected @endif>{{ Task::getStatus(Task::NOTCOMPLETE) }}</option>
                                </select>
                            </td>
                            <td>
